...
**なお、この無音は「Binance が配信していない」の証拠にはならない。**
`/ws/` が旧パスで、市場データが `/market/ws/` へ移っている可能性がある。
depth だけ届いて他が無音、という症状はパスの陳腐化でも説明がつく。
`tools/probe_market_path.py` が両者を1本ずつ接続して判定する。
**REST 迂回は残すが、既定に据える前にこれを確認すること。**
...
- **清算データの扱い（前の記述は誤り）。** Binance は USDⓈ-M に清算ストリームを
  提供している（`<sym>@forceOrder` / 全銘柄 `!forceOrder@arr`）。ただし
  **1銘柄1秒につき最新1件のみのスナップショット**なので、清算が集中した局面では
  途中が欠落する。「取得不能」ではなく「一部取得できるが完全なテープではない」。
  当環境で無音だったのは接続先の問題の可能性が高い
