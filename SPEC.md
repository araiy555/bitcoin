...
#### パスを間違えると静かに壊れる（実測で解決済み）

**市場データは `/market/stream` と `/market/ws` にある。**
旧パスの `/stream` と `/ws` は depth と bookTicker だけを配信し、
他は**エラーも切断も出さずに無音**になる。`LIST_SUBSCRIPTIONS` は
それでも全ストリームを「有効」と返す。
...
|---|---|---|
| `aggTrade` | 36件 | **0件** |
| `markPrice@1s` | 15件（1秒1件） | — |
| `!forceOrder@arr` | 2件 | — |
| `depth@100ms` | — | 146件 |

**この誤りに1日を費やした。** 「取引所が配信していない」と読んだが、
実際は古いパスに投げていただけだった。ここから2つ：

- **接続でき、購読が承認され、一部のストリームが届くことは、
  残りが取得不能である証拠にならない**
- REST 迂回は障害時のバックアップであって、常用経路ではない

`tools/probe_market_path.py` がこの判定を再現する。
