...
板の歪み特徴量・方向予測はこの上に載る予定で、まだ存在しない（§15）。
...
#### 配信されないストリームへの対処（実測に基づく）

`fstream.binance.com` に対する実測：**購読は受理され、`LIST_SUBSCRIPTIONS`
が4本すべてを「有効」として返し、それでも `aggTrade` / `markPrice` /
`forceOrder` が1件も来ない。** `depth` と `bookTicker` は正常に届く。
現物に同じクライアントで同じ手順を踏むと全部返ってくる。

リクエストの不備でも、静かな市場でもない。`markPrice@1s` は相場と無関係の
1秒定期配信で、稼働銘柄で12秒間無音はあり得ない。
...
- **清算データが取れない。** `forceOrder` が配信されず REST 代替も無いため、
  この環境では清算は記録できない。清算カスケードを使う特徴量は作れない
