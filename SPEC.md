...
| `test_futures_sync.py` | 22 | **先物の継ぎ目（`pu` 規則）**、マーク/清算/建玉のパーサ |
...
- **`capture` の正常系も同じで、こちらはまだ一度も実データに当てていない。**
  先物の継ぎ目・各パーサ・複数銘柄の同時記録はフェイクソケットと
  スクリプト化フィードで検証済みだが、実際の fapi / fstream には未接続
  （開発環境から binance.com への通信が塞がれているため）。
  最初の実行は短い `--duration` にして、両方の `src` が届いているか、
  `resyncing` が頻発していないかを確認すること
